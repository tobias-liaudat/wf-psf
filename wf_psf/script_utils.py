...
def train_model(**args):
    ...
    if args["pretrained_model"] is not None:
        ...
        if args["project_dd_features"]:
            wf_utils.manage_project_DD_features(
                tf_semiparam_field, args["model"], tf_zernike_cube
            )
    ...
    if args["model"] == "param":
        ...
        tf_semiparam_field, hist_param, hist_non_param = (
    ...
    while args["total_cycles"] > current_cycle:
        ...
        if args["model"] == "param":
            ...
            # Compute the next cycle
            tf_semiparam_field, hist_param_2, hist_non_param_2 = (
                wf_train_utils.general_train_cycle(
                    tf_semiparam_field,
                    inputs=inputs,
                    outputs=outputs,
...
def evaluate_model(**args):
    ...
    if args["saved_model_type"] == "checkpoint":
        ...
        model_save_file = args["base_path"] + args["model_folder"]
        weights_paths = (
            model_save_file + "chkp_" + run_id_name + "_" + args["saved_cycle"]
        )
    ...
    try:
        ...
        if args["model_eval"] == "physical":
            ...
            all_zernike_GT = np.concatenate(
                (train_dataset["zernike_GT"], test_dataset["zernike_GT"]), axis=0
            )
            all_pos = np.concatenate(
                (train_dataset["positions"], test_dataset["positions"]), axis=0
            )
    ...
    for plot_dataset in ["test", "train"]:
        ...
        if args["eval_mono_metric_rmse"] is True or "eval_mono_metric_rmse" not in args:
            try:
                ...
                ax2 = ax1.twinx()
                kwargs = dict(
                    linewidth=2, linestyle="dashed", markersize=8, marker="^", alpha=0.5
                )
                for it in range(n_datasets):
                    ax2.plot(
                        lambda_list,
...
def extract_poly_results(metrics_dicts, test_train="test"):
    ...
    for it in range(n_dicts):
        ...
        polyc_std_rel_rmse[it] = metrics_dicts[it][first_key]["poly_metric"][
            "std_rel_rmse"
        ]
...
def extract_opd_results(metrics_dicts, test_train="test"):
    ...
    for it in range(n_dicts):
        ...
        opd_std_rel_rmse[it] = metrics_dicts[it][first_key]["opd_metric"][
            "rel_rmse_std_opd"
        ]
...
def extract_shape_pix_results(metrics_dicts, test_train="test"):
    ...
    for it in range(n_dicts):
        ...
        polyc_std_rmse[it] = metrics_dicts[it][first_key]["shape_results_dict"][
            "pix_rmse_std"
        ]
        polyc_rel_rmse[it] = metrics_dicts[it][first_key]["shape_results_dict"][
            "rel_pix_rmse"
        ]
